...
if (!$slip) {
    header('Location: payslips.php?error=not_found');
    exit;
}
...
if ($auth->isAdmin()) {
    ...
    if (!$employee || $slip['employee_id'] != $employee['id']) {
        header('Location: payslips.php?error=unauthorized');
        exit;
    }
}

$payroll->recordPayslipView($payroll_id, $_SESSION['user_id']);
$current_page = 'payslips';

// Get earnings, adjustments, deductions data
...
<head>
    ...
    <title>View Payslip - Natcorp Payroll System</title>
...
<body>
    ...
    <div class="main-content">
        <div class="page-header">
            <div>
                <h1>View Payslip</h1>
                <p class="subtitle"><?php echo date('j F Y', mktime(0, 0, 0, $slip['month'], $slip['day'] ?? 1, $slip['year'])); ?></p>
            </div>
            <div>
                <a href="print_payslip.php?id=<?php echo $payroll_id; ?>" class="btn btn-primary" target="_blank">Print</a>
                <a href="payslips.php" class="btn btn-secondary">Back</a>
            </div>
        </div>
                ...
                <div class="payslip-topbar">
                    <div class="company-brand">
                        ...
                        <div class="company-brand-text">
                            ...
                            <div class="company-subtitle">NATCORP CAREER GROWTH AND MANPOWER SERVICES</div>
                            <div class="company-address">Unit C, 2nd FL Cedilla Bldg Magsay Road, Brgy. Parang, Cainta, Rizal</div>
                        </div>
                    </div>
                    <div class="payroll-meta">
                        <span class="label">Payroll Date</span>
                        <span class="value"><?php echo strtoupper(date('j F Y', mktime(0, 0, 0, $slip['month'], $slip['day'] ?? 1, $slip['year']))); ?></span>
                    </div>
                </div>
